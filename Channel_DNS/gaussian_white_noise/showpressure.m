...
P_error = @(P) std(P-P_ref, 1, 'all')/(0.5*1^2);
...
% fprintf(['pressure error from U-Net filtered field: ',...
%     num2str(P_error(P_unet)),'\n']);
...
%     num2str(P_simi(P_ae)),'\n']);
% fprintf(['pressure similarity from U-Net filtered field to reference: ',...
%     num2str(P_simi(P_unet)),'\n']);